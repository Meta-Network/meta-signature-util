 ...
 * If a payload contains other params, pass it as the second param.
 * @param {AuthorDigestRequestMetadata} metadata to verify
...
export const verifyAuthorDigestMetadataSignature = (
  metadata: AuthorSignatureMetadata,
): boolean => {
  return verifySignature(metadata, { digest: metadata.digest });
};

export const verifyAuthorPublishMetaSpaceRequestMetadataSignature = (
  metadata: AuthorSignatureMetadata,
): boolean => {
  return verifySignature(metadata);
};

export const verifyServerMetadataSignature = (
  metadata: SignatureMetadata,
): boolean => {
  return verifySignature(metadata, {
    signature: (metadata.reference.slice(-1)[0].body as SignatureMetadata)
      .signature,
  });
};
